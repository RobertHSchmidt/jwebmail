/* CVS ID: $Id$ */
package net.wastl.webmail.session.user;
...
import net.wastl.webmail.storage.data.XMLUserData;
...
//              }
                ...
                if(msgs[i].getSubject() != null) {
                    try {
                        ...
                        parent.getLogger().log(Logger.LOG_WARN,"Unsupported Encoding: "+ex.getMessage());
                    }
                }
                ...
                try {
                    ...
                    parent.getLogger().log(Logger.LOG_WARN,
            ...
            parent.getLogger().log(Logger.LOG_DEBUG,"Construction of message list took "+(time_stop-time_start)+" ms. Time for IMAP transfer was "+(fetch_stop-fetch_start)+" ms.");
            folder.close(false);
     ...
     * @see net.wastl.webmail.server.UserSession.GETMESSAGE_MODE_STANDARD
     * @see net.wastl.webmail.server.UserSession.GETMESSAGE_MODE_REPLY
     * @see net.wastl.webmail.server.UserSession.GETMESSAGE_MODE_FORWARD
     */
    ...
    protected Store connectStore(String host,String protocol,String login, String password) throws MessagingException {
        ...
        if(!st.isConnected()) {
            try {
                ...
                parent.getLogger().log(Logger.LOG_INFO,"Mail: Connection to "+st.toString()+".");
                ...
                if(!login_password.equals(password) &&
                   parent.getStorage().getConfig("FOLDER TRY LOGIN PASSWORD").toUpperCase().equals("YES")) {
                    ...
                    parent.getLogger().log(Logger.LOG_INFO,"Mail: Connection to "+st.toString()+", second attempt with login password succeeded.");
                } else {
                    throw ex;
                }
            }
    ...
    */
    public void disconnect(String name) {
        try {
            ...
            if(f != null && f.isOpen()) {
                ...
                //st.close();
                parent.getLogger().log(Logger.LOG_INFO,"Mail: Disconnected from folder "+f.toString()+" at store "+st.toString()+".");
            } else {
                parent.getLogger().log(Logger.LOG_WARN,"Mail: Folder "+name+" was null???.");
            }
        } catch(MessagingException ex) {
            // Should not happen
            ex.printStackTrace();
    ...
    public void saveData() {
        try {
            ...
            parent.getLogger().log(Logger.LOG_ERR,"Could not save user data");
