...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" version="1.0">
  ...
  <xsl:template match="MESSAGELIST">
	<TR>
	  ...
	  <TD  width="7%" class="testoGrande" bgcolor="#A6B1C0">Mark.</TD>
	  <TD width="28%" class="testoGrande" bgcolor="#A6B1C0">Betreff</TD>
	  <TD width="25%" class="testoGrande" bgcolor="#A6B1C0">Absender</TD>
	  <TD width="25%" class="testoGrande" bgcolor="#A6B1C0">Datum</TD>
	  <TD width="5%" class="testoGrande" bgcolor="#A6B1C0">Größe</TD>
	</TR>
	...
	<TR>
		...
		<xsl:for-each select="/USERMODEL/MAILHOST_MODEL//FOLDER">
	      ...
	      <INPUT TYPE="SUBMIT" NAME="copymovemsgs" VALUE="Ausführen!" class="testoNero"/>
	    </TD>
	</TR>
  </xsl:template>
	...
	<TR>
	  <TD colspan="7" width="100%" height="22" class="testoNero" align="center">
		...
		<xsl:when test="number(/USERMODEL/CURRENT[@type='folder']/@first_msg) > number(1)">
		  ...
		  <TD class="testoBianco" width="47%" bgcolor="#697791" valign="middle"><A HREF="{$base}/folder/list?session-id={$session-id}&amp;folder-id={@id}&amp;part={/USERMODEL/CURRENT[@type='folder']/@list_part + 1}"><SPAN class="testoBianco"> Vorige <xsl:value-of select="/USERMODEL/USERDATA/INTVAR[@name='max show messages']/@value"/> Nachrichten</SPAN></A>
		  </TD>
		</xsl:when>
		<xsl:otherwise>
		...
		<xsl:when test="number(/USERMODEL/CURRENT[@type='folder']/@list_part) > number(1)">
		  <TD class="testoBianco" width="47%" bgcolor="#697791" valign="middle" align="right">
		    <A HREF="{$base}/folder/list?session-id={$session-id}&amp;folder-id={@id}&amp;part={/USERMODEL/CURRENT[@type='folder']/@list_part - 1}"><SPAN class="testoBianco"> Nächste <xsl:value-of select="/USERMODEL/USERDATA/INTVAR[@name='max show messages']/@value"/> Nachrichten</SPAN></A>
		   </TD>
		   <TD class="testoBianco" width="3%" bgcolor="#697791" valign="middle" align="center">
		   <A HREF="{$base}/folder/list?session-id={$session-id}&amp;folder-id={@id}&amp;part={/USERMODEL/CURRENT[@type='folder']/@list_part - 1}">
