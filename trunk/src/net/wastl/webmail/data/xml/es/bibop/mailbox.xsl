  ...
  <xsl:output method="html" indent="yes" xalan:content-handler="org.apache.xml.serializer.ToHTMLStream"/>
    ...
    <HTML>
      ...
      <BODY bgcolor="#B5C1CF" topmargin="5" leftmargin="0" marginwidth="0" marginheight="5">
        <P class="testoChiaro">
          <SPAN class="testoScuro">Bienvenido a tu buz&#243;n, <xsl:value-of select="/USERMODEL/USERDATA/FULL_NAME"/>.</SPAN><BR/>
          Esta es la <SPAN class="testoScuro"><xsl:apply-templates select="/USERMODEL/USERDATA/INTVAR[@name='login count']"/>
          vez</SPAN> que te conectas desde el
          <SPAN class="testoScuro"><xsl:apply-templates select="/USERMODEL/STATEDATA/VAR[@name='first login']"/></SPAN>.
          La <SPAN class="bold">&#250;ltima vez que te conectaste</SPAN> fue el
          ...
          (el total de mensajes est&#225; en <SPAN class="testoVerde">verde</SPAN>,
          los nuevos mensajes en <SPAN class="testoRosso">rojo</SPAN>) (<A HREF="{$base}/help?session-id={$session-id}&amp;helptopic=mailbox">Ayuda</A>):<BR/><BR/>
              <A HREF="{$base}/mailbox?session-id={$session-id}&amp;force-refresh=1">Forzar actualizaci&#243;n</A> - Haz click aqu&#237; para forzar a que se actualice la informaci&#243;n de las carpetas.
        </P>
  ...
  <xsl:template match="/USERMODEL/MAILHOST_MODEL">
    <TR>
      ...
      <TD bgcolor="#909CAF" width="48%" class="testoScuro">
        Servidor: <xsl:value-of select="@url"/>
      </TD>
      ...
      <xsl:choose>
        <xsl:when test="@holds_messages = 'true'">
          ...
          <TD WIDTH="48%" bgcolor="#D3D8DE" valign="middle" class="testoNero">
            <SPAN class="testoVerde"><xsl:value-of select="MESSAGELIST/@total"/></SPAN>/<SPAN class="testoRosso"><xsl:value-of select="MESSAGELIST/@new"/></SPAN> mensajes
          </TD>
