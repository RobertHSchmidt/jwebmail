...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="/">
    <HTML>
      <HEAD>
        <TITLE>Buz&#243;n de JWebMail para <xsl:value-of select="/USERMODEL/USERDATA/FULL_NAME"/>: Redactar mensaje <xsl:value-of select="/USERMODEL/CURRENT[@type='message']/@id"/></TITLE>
        ...
        <FORM ACTION="{$base}/send?session-id={$session-id}" METHOD="POST">
          <TABLE BGCOLOR="#dddddd" CELLSPACING="0" CELLPADDING="5" BORDER="0">
            <TR>
              ...
              <TD VALIGN="CENTER"><FONT SIZE="+2"><STRONG>Redactar mensaje</STRONG></FONT> (<A HREF="{$base}/help?session-id={$session-id}&amp;helptopic=compose">Ayuda</A>)<BR/><EM>Fecha: <xsl:value-of select="/USERMODEL/STATEDATA/VAR[@name='date']/@value"/></EM></TD>
            </TR>
            <TR>
              <TD>
                <STRONG>Para:</STRONG>
