...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" version="1.0">
  <xsl:output method="html" indent="no" encoding="UTF-8" xalan:content-handler="org.apache.xalan.serialize.SerializerToHTML"/>
...
<IMG SRC="{$imgbase}/images/icona_mail.gif" BORDER="0" hspace="5" align="absmiddle"/><SPAN class="testoNero">Zeige Nachricht (<A HREF="{$base}/help?session-id={$session-id}&amp;helptopic=messagelist">Hilfe</A>)</SPAN>
  ...
  <xsl:template match="MESSAGE">
    ...
    <TABLE width="100%" border="0" cellspacing="2" cellpadding="4">
      ...
      <TR>
        <TD height="22" class="testoGrande" width="13%" bgcolor="#E2E6F0">Von:
        </TD>
      ...
      <TR>
        <TD height="22" class="testoGrande" width="13%" bgcolor="#E2E6F0">An:
        </TD>
        <TD height="22" class="testoNero" width="87%" bgcolor="#D1D7E7">
      ...
      <TR>
        <TD height="22" class="testoGrande" width="13%" bgcolor="#E2E6F0">Antwort An:
        </TD>
        <TD height="22" class="testoNero" width="87%" bgcolor="#D1D7E7">
      ...
      <TR>
        <TD height="22" class="testoGrande" width="13%" bgcolor="#E2E6F0">Kopie:
        </TD>
        <TD height="22" class="testoNero" width="87%" bgcolor="#D1D7E7">
      ...
      <TR>
        <TD height="22" class="testoGrande" width="13%" bgcolor="#E2E6F0">Datum:
        </TD>
        <TD height="22" class="testoNero" width="87%" bgcolor="#D1D7E7">
      ...
      <TR>
        <FORM ACTION="{$base}/folder/list?flag=1&amp;session-id={$session-id}&amp;folder-id={/USERMODEL/CURRENT[@type='folder']/@id}" METHOD="POST">
            <TD align="center" class="testoNero">
              ...
              <INPUT TYPE="SUBMIT" NAME="flagmsgs" VALUE="Ausführen!" class="testoNero"/>
            </TD>
          </FORM>
  ...
  <xsl:template match="PART">
    <xsl:choose>
      ...
      <xsl:when test="@type='multi'">
        <xsl:for-each select="PART">
          <CENTER><P class="testoGrande">MIME Abschnitt</P></CENTER><BR/>
          <xsl:apply-templates select="."/>
        </xsl:for-each>
      </xsl:when>
      <xsl:otherwise>
        <TABLE width="100%" border="0" cellspacing="2" cellpadding="4">
          <TR>
            <TD height="22" class="testoGrande" width="13%" bgcolor="#E2E6F0">Angehängte Datei
            </TD>
            <TD height="22" class="testoNero" width="87%" bgcolor="#D1D7E7">
          ...
          <TR>
            <TD height="22" class="testoGrande" width="13%" bgcolor="#E2E6F0">Beschreibung:</TD>
            <TD height="22" class="testoNero" width="87%" bgcolor="#D1D7E7">
          ...
          <TR>
            <TD height="22" class="testoGrande" width="13%" bgcolor="#E2E6F0">Typ:</TD>
            <TD height="22" class="testoNero" width="87%" bgcolor="#D1D7E7"><xsl:value-of select="@content-type"/></TD>
          </TR>
          <TR>
            <TD height="22" class="testoGrande" width="13%" bgcolor="#E2E6F0">Größe:</TD>
          ...
          <xsl:choose>
             <xsl:when test="@msgnr > 1">
                ...
                <TD width="30%" bgcolor="#697791" valign="middle">
                  <A HREF="{$base}/folder/showmsg?session-id={$session-id}&amp;folder-id={/USERMODEL/CURRENT[@type='folder']/@id}&amp;message-nr={@msgnr - 1}"> <SPAN class="testoBianco">Vorige Nachricht</SPAN>
                  </A>
                </TD>
                </xsl:when>
                <xsl:otherwise>
                ...
                <TD width="30%" bgcolor="#697791" valign="middle" align="center">
                  <A HREF="{$base}/folder/list?session-id={$session-id}&amp;folder-id={/USERMODEL/CURRENT[@type='folder']/@id}&amp;part={/USERMODEL/CURRENT[@type='folder']/@list_part}">
                    ...
                    <BR/><SPAN class="testoBianco">Nachrichtenliste</SPAN>
                 </A>
                </TD>
            <xsl:variable name="current_folder" select="/USERMODEL/CURRENT[@type='folder']/@id"/>
              <xsl:choose>
                ...
                <TD width="30%" bgcolor="#697791" valign="middle" align="right">
                 <A HREF="{$base}/folder/showmsg?session-id={$session-id}&amp;folder-id={/USERMODEL/CURRENT[@type='folder']/@id}&amp;message-nr={@msgnr + 1}">
                   <SPAN class="testoBianco">Nächste Nachricht</SPAN>
                 </A>
                </TD>
                <TD width="5%" bgcolor="#697791" valign="middle" align="center">
                  <A HREF="{$base}/folder/showmsg?session-id={$session-id}&amp;folder-id={/USERMODEL/CURRENT[@type='folder']/@id}&amp;message-nr={@msgnr + 1}">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <TABLE  BORDER="1" WIDTH="100%" CELLSPACING="0" CELLPADDING="0">
          <TR>
            <TD BGCOLOR="#FF9933">
              Nicht vertrauenswürdiges HTML element entfernt: <xsl:value-of select="@malicious"/>
            </TD>
          </TR>
          <TR>
            <TD bgcolor="#3399FF" >
