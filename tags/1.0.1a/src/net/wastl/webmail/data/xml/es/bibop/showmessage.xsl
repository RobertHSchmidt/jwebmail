...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" version="1.0">
  <xsl:output method="html" indent="no" xalan:content-handler="org.apache.xalan.serialize.SerializerToHTML"/>
    ...
    <HTML>
      <HEAD>
        <TITLE>Buz&#243;n de WebMail para <xsl:value-of select="/USERMODEL/USERDATA/FULL_NAME"/>: Mostrar mensaje <xsl:value-of select="/USERMODEL/CURRENT[@type='message']/@id"/></TITLE>
        <META CONTENT="AUTHOR" VALUE="Sebastian Schaffert"/>
        <link rel="stylesheet" href="{$base}/passthrough/webmail.css"/>
      </HEAD>

      <BODY bgcolor="#B5C1CF" topmargin="5" leftmargin="0" marginwidth="0" marginheight="5">
<IMG SRC="{$imgbase}/images/icona_mail.gif" BORDER="0" hspace="5" align="absmiddle"/><SPAN class="testoNero">Mostrar mensaje (<A HREF="{$base}/help?session-id={$session-id}&amp;helptopic=messagelist">Ayuda</A>)</SPAN>
  ...
  <xsl:template match="MESSAGE">
    ...
    <TABLE width="100%" border="0" cellspacing="2" cellpadding="4">
      ...
      <TR>
        <TD height="22" class="testoGrande" width="13%" bgcolor="#E2E6F0">Para:
        </TD>
        <TD height="22" class="testoNero" width="87%" bgcolor="#D1D7E7">
      ...
      <TR>
        <TD height="22" class="testoGrande" width="13%" bgcolor="#E2E6F0">Responder a:
        </TD>
        <TD height="22" class="testoNero" width="87%" bgcolor="#D1D7E7">
      ...
      <TR>
        <TD height="22" class="testoGrande" width="13%" bgcolor="#E2E6F0">Fecha:
        </TD>
        <TD height="22" class="testoNero" width="87%" bgcolor="#D1D7E7">
      ...
      <TR>
        <TD COLSPAN="4" bgcolor="A6B1C0" ALIGN="right" class="testoNero">
          <A HREF="{$base}/compose?session-id={$session-id}&amp;folder-id={/USERMODEL/CURRENT[@type='folder']/@id}&amp;message-nr={@msgnr}&amp;reply=1">responder a este mensaje...</A> -
          <A HREF="{$base}/compose?session-id={$session-id}&amp;folder-id={/USERMODEL/CURRENT[@type='folder']/@id}&amp;message-nr={@msgnr}&amp;forward=1">reenviar este mensaje...</A>
        </TD>
      </TR>
    </TABLE>
      ...
      <TR>
        <FORM ACTION="{$base}/folder/list?flag=1&amp;session-id={$session-id}&amp;folder-id={/USERMODEL/CURRENT[@type='folder']/@id}" METHOD="POST">
            <TD align="center" class="testoNero">
              ...
              <INPUT TYPE="SUBMIT" NAME="flagmsgs" VALUE="Perform!" class="testoNero"/>
            </TD>
          </FORM>
          <FORM ACTION="{$base}/folder/showmsg?flag=1&amp;session-id={$session-id}&amp;folder-id={/USERMODEL/CURRENT[@type='folder']/@id}" METHOD="POST">
            <TD align="center" class="testoNero">
              ...
              <INPUT TYPE="SUBMIT" NAME="copymovemsgs" VALUE="Perform!" class="testoNero"/>
            </TD>
         </FORM>
       </TR>
    </TABLE>
  ...
  <xsl:template match="PART">
    <xsl:choose>
      ...
      <xsl:when test="@type='multi'">
        <xsl:for-each select="PART">
          <CENTER><P class="testoGrande">fragmento MIME</P></CENTER><BR/>
          <xsl:apply-templates select="."/>
        </xsl:for-each>
      </xsl:when>
      <xsl:otherwise>
        <TABLE width="100%" border="0" cellspacing="2" cellpadding="4">
          <TR>
            <TD height="22" class="testoGrande" width="13%" bgcolor="#E2E6F0">Fichero adjunto
            </TD>
            <TD height="22" class="testoNero" width="87%" bgcolor="#D1D7E7">
          ...
          <TR>
            <TD height="22" class="testoGrande" width="13%" bgcolor="#E2E6F0">Descripci&#243;n:</TD>
            <TD height="22" class="testoNero" width="87%" bgcolor="#D1D7E7">
          ...
          <TR>
            <TD height="22" class="testoGrande" width="13%" bgcolor="#E2E6F0">Tipo:</TD>
            <TD height="22" class="testoNero" width="87%" bgcolor="#D1D7E7"><xsl:value-of select="@content-type"/></TD>
          </TR>
          <TR>
            <TD height="22" class="testoGrande" width="13%" bgcolor="#E2E6F0">Tama&#241;o:</TD>
          ...
          <xsl:choose>
             <xsl:when test="@msgnr > 1">
                ...
                <TD width="30%" bgcolor="#697791" valign="middle">
                  <A HREF="{$base}/folder/showmsg?session-id={$session-id}&amp;folder-id={/USERMODEL/CURRENT[@type='folder']/@id}&amp;message-nr={@msgnr - 1}"> <SPAN class="testoBianco">Mensaje anterior</SPAN>
                  </A>
                </TD>
                </xsl:when>
                <xsl:otherwise>
                ...
                <TD width="30%" bgcolor="#697791" valign="middle" align="center">
                  <A HREF="{$base}/folder/list?session-id={$session-id}&amp;folder-id={/USERMODEL/CURRENT[@type='folder']/@id}&amp;part={/USERMODEL/CURRENT[@type='folder']/@list_part}">
                    ...
                    <BR/><SPAN class="testoBianco">lista de mensajes</SPAN>
                 </A>
                </TD>
            <xsl:variable name="current_folder" select="/USERMODEL/CURRENT[@type='folder']/@id"/>
              <xsl:choose>
                ...
                <TD width="30%" bgcolor="#697791" valign="middle" align="right">
                 <A HREF="{$base}/folder/showmsg?session-id={$session-id}&amp;folder-id={/USERMODEL/CURRENT[@type='folder']/@id}&amp;message-nr={@msgnr + 1}">
                   <SPAN class="testoBianco">Siguiente mensaje</SPAN>
                 </A>
                </TD>
                <TD width="5%" bgcolor="#697791" valign="middle" align="center">
                  <A HREF="{$base}/folder/showmsg?session-id={$session-id}&amp;folder-id={/USERMODEL/CURRENT[@type='folder']/@id}&amp;message-nr={@msgnr + 1}">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <TABLE  BORDER="1" WIDTH="100%" CELLSPACING="0" CELLPADDING="0">
          <TR>
            <TD BGCOLOR="#FF9933">
              Elemento HTML no fiable eliminado: <xsl:value-of select="@malicious"/>
            </TD>
          </TR>
          <TR>
            <TD bgcolor="#3399FF" >
