...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" version="1.0">
  <xsl:output method="html" indent="yes" xalan:content-handler="org.apache.xml.serializer.ToHTMLStream"/>
    ...
    <HTML>
      <HEAD>
        <TITLE>Buz&#243;n de WebMail para <xsl:value-of select="/USERMODEL/USERDATA/FULL_NAME"/>: Lista de mensajes (Carpeta <xsl:value-of select="/USERMODEL/CURRENT[@type='folder']/@id"/></TITLE>
  ...
  <xsl:template match="MESSAGELIST">
        <TR>
          ...
          <TD width="5%" class="testoGrande" bgcolor="#A6B1C0">Nr</TD>
          <TD  width="7%" class="testoGrande" bgcolor="#A6B1C0">Marcas</TD>
          <TD width="28%" class="testoGrande" bgcolor="#A6B1C0">Asunto</TD>
          <TD width="25%" class="testoGrande" bgcolor="#A6B1C0">Remitente</TD>
          <TD width="25%" class="testoGrande" bgcolor="#A6B1C0">Fecha</TD>
          <TD width="5%" class="testoGrande" bgcolor="#A6B1C0">Tama&#241;o</TD>
        </TR>
              ...
              <TR>
               <xsl:choose>
                <xsl:when test="number(/USERMODEL/CURRENT[@type='folder']/@first_msg) > number(1)">
                  ...
                  <TD class="testoBianco" width="47%" bgcolor="#697791" valign="middle"><A HREF="{$base}/folder/list?session-id={$session-id}&amp;folder-id={@id}&amp;part={/USERMODEL/CURRENT[@type='folder']/@list_part + 1}"><SPAN class="testoBianco"> Anteriores <xsl:value-of select="/USERMODEL/USERDATA/INTVAR[@name='max show messages']/@value"/> mensajes</SPAN></A>
                  </TD>
                </xsl:when>
                <xsl:otherwise>
              ...
              <xsl:choose>
                <xsl:when test="number(/USERMODEL/CURRENT[@type='folder']/@list_part) > number(1)">
                  <TD class="testoBianco" width="47%" bgcolor="#697791" valign="middle" align="right">
                    <A HREF="{$base}/folder/list?session-id={$session-id}&amp;folder-id={@id}&amp;part={/USERMODEL/CURRENT[@type='folder']/@list_part - 1}"><SPAN class="testoBianco"> Siguientes <xsl:value-of select="/USERMODEL/USERDATA/INTVAR[@name='max show messages']/@value"/> mensajes</SPAN></A>
                   </TD>
                   <TD class="testoBianco" width="3%" bgcolor="#697791" valign="middle" align="center">
                   <A HREF="{$base}/folder/list?session-id={$session-id}&amp;folder-id={@id}&amp;part={/USERMODEL/CURRENT[@type='folder']/@list_part - 1}">
  ...
  <xsl:template match="FROM">
      <xsl:choose>
        <xsl:when test="contains(.,'&lt;')">
          ...
          <SPAN class="bold">Correo:</SPAN> <xsl:value-of select="substring-before(substring-after(.,'&lt;'),'>')"/>
        </xsl:when>
        <xsl:otherwise>
          <BR/>
          <SPAN class="bold">Correo:</SPAN> <xsl:value-of select="."/>
        </xsl:otherwise>
      </xsl:choose>
  </xsl:template>
</xsl:stylesheet>
